...
class UnreachPosture(BaseTerminationCondition):
    ...
    def __init__(self, config, device):
        ...
        self.reach_posture_steps = getattr(config, 'reach_posture_steps', 10)
    ...
    def get_termination(self, task, env, info={}):
        ...
        mask3 = torch.abs(wrap_PI(heading) - task.target_heading) >= torch.pi / 36    # 5°
        # 判断是否到达target_pitch
        mask4 = torch.abs(wrap_PI(pitch) - task.target_pitch) >= torch.pi / 36        # 5°
        # 判断是否到达target_vt
        # mask5  = torch.abs(vt - task.target_vt) >= 20                                 # 20 feet/s
        mask5  = torch.abs(vt - task.target_vt) >= 10                                 # 10 feet/s
        # 当超过时间且未达到目标时，判断为True
        bad_done = mask1 & ((mask3 | mask4) | mask5)
        # 当达到目标且时间符合要求时，重新设置目标
        mask =  ((~((mask3 | mask4) | mask5)) & (~mask1))
        task.reach_posture_steps[mask] += 1
        task.reach_posture_steps[~mask] = 0
        done = task.reach_posture_steps > self.reach_posture_steps
        exceed_time_limit = torch.zeros_like(done)
        
        if torch.any(bad_done):
            # self.log(f'unreach posture!')
            info["done"]["UnreachPosture"] = torch.sum(bad_done).item()
            # print(torch.sum(bad_done), 'unreach posture!')
        if torch.any(done):
            # self.log(f'reset target!')
            info["done"]["ReachPosture"] = torch.sum(done).item()
            # print(torch.sum(done), 'reset target!')
        
        return bad_done, done, exceed_time_limit, info

    def get_termination_0108(self, task, env, info={}):
        """
        Return whether the episode should terminate.
        End up the simulation if the aircraft didn't reach the target in limited time.
